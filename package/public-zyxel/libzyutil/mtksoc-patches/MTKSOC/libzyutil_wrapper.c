...
void zyUtilGetMrdInfoFilePath(char* MrdInfoFilePath)
{
	sprintf(MrdInfoFilePath, "%s", MRDINFO_FILE);
}
...
zcfgRet_t zyUtilGetMrdInfo(PMRD_DATA mrd)
{
	...
	if (access(MRDINFO_FILE, F_OK) != 0)
    {
		// Get MRD info from factory partition with offset 0xfe00(MSTC)
		sprintf(sysCmd,FLASH_READ_CMD,MRDINFO_FILE,(long unsigned int)MRDINFO_SIZE,(long unsigned int)MRDINFO_OFFSET,MRDINFO_PARTITON_NAME);
		system(sysCmd);
		sync ();
...
zcfgRet_t zyUtilSetMrdInfo(PMRD_DATA mrd)
{
	...
	// Set MRD info from file into factory partition at offset 0xfe00(MSTC)
...
#if defined (ZYXEL_SUPPORT_SAS_CBSD)
#define MAX_LENGTH_OF_PATH	4096
#define TEMP_CERT_PATH 		"/tmp/temp_cert.pem"
#define TEMP_KEY_PATH 		"/tmp/temp_cert.pem.key"
zcfgRet_t zyUtilGetCBSDCertificate(char *cert)
{
	...
	ret=nvram_file_get (UBOOT_EXTRA_ENV_LOCAL_CERTIFICATE, LOCAL_CERTIFICATE, UBOOT_STARTUP_FILE);

	if (ret) {
		zyUtilGetMrdInfo(&mrd_buff);
		snprintf(cert, MAXLEN_CERT, "%s", mrd_buff.Certificate);
		ret = 0;
	}
	else{
		sprintf(cert ,"%s",	LOCAL_CERTIFICATE);
	}
...
zcfgRet_t zyUtilGetCBSDPrivateKey(char *privkey)
{
	...
	ret=nvram_file_get (UBOOT_EXTRA_ENV_PRIVATE_KEY, PRIVATE_KEY, UBOOT_STARTUP_FILE);

	if (ret) {
		zyUtilGetMrdInfo(&mrd_buff);
		snprintf(privkey, MAXLEN_CERT, "%s", mrd_buff.PrivateKey);
		ret = 0;
	}
	else{
		sprintf(privkey ,"%s",	PRIVATE_KEY);
	}
...
zcfgRet_t zyUtilWriteCBSDCertKey(char *file_name, char *password)
{
	struct mrd mrd_buff;
	int32_t ret;
	FILE *fpCert, *fpKey = NULL;
	char buf[MAXLEN_CERT];
	char linebuf[256];
	bool startPub = false;
	...
	if (strlen(file_name) > MAX_LENGTH_OF_PATH)
	{
	...
	ret = CertPrivateKeyConvertHandler(file_name, password);
	...
	if(ret){
		...
		while(fgets(linebuf, sizeof(linebuf), fpCert)){
		...
		if((sizeof(buf)-strlen(buf)-1) > 0){
			memset(buf+strlen(buf)+1, 0xff, sizeof(buf)-strlen(buf)-1);
		}
		memcpy(mrd_buff.Certificate, buf, sizeof(buf));

		memset(buf, 0, sizeof(buf));
		memset(linebuf, 0, sizeof(linebuf));
		while(fgets(linebuf, sizeof(linebuf), fpKey)){
			...
			if(strstr(linebuf,"-----END RSA PRIVATE KEY-----") || strstr(linebuf,"-----END PRIVATE KEY-----")){
				startPriv = false;
			}
			memset(linebuf, 0, sizeof(linebuf));
		}
		...
		memcpy(mrd_buff.PrivateKey, buf, sizeof(buf));
		
		fclose(fpCert);
		fclose(fpKey);

		if (zyUtilSetMrdInfo(&mrd_buff) != 0) {
			return ZCFG_SUCCESS;
		}
...
zcfgRet_t CertPrivateKeyConvertHandler(char *filePath, char *password)
{
	...
	/* Check wherever same file name exist */
	if(stat(TEMP_CERT_PATH, &fileStat) == 0)
		remove(TEMP_CERT_PATH);
	if(stat(TEMP_KEY_PATH, &fileStat) == 0)
		remove(TEMP_KEY_PATH);

	if(password != NULL && strlen(password) > 0){
		//Remain chain certificate. Do net use -clcerts option.
		sprintf(cmd, "openssl pkcs12 -in %s -nodes -out %s -password pass:%s",
			filePath, TEMP_CERT_PATH, password);
	}else{
		sprintf(cmd, "openssl pkcs12 -in %s -clcerts -nodes -out %s -password pass:", 
			filePath, TEMP_CERT_PATH);
	}
	system(cmd);
	...
	//Get privatekey from combined .pem file.
	if (result){ //parse key file, should always success here since pkcs12 already success.
		sprintf(cmd, "openssl rsa -in %s -out %s", TEMP_CERT_PATH, TEMP_KEY_PATH);
		system(cmd);
	}
		
	//try PEM
	if(result == 0){
		sprintf(cmd, "openssl crl2pkcs7 -nocrl -certfile %s | openssl pkcs7 -print_certs -out %s", filePath, TEMP_CERT_PATH);
		system(cmd);
		...
		if (result){
			if(password != NULL && strlen(password) > 0){
				sprintf(cmd, "openssl rsa -in %s -out %s -passin pass:%s", filePath, TEMP_KEY_PATH, password);
			}else{
				sprintf(cmd, "openssl rsa -in %s -out %s -passin pass:", filePath, TEMP_KEY_PATH);
			}
			//printf("cmd:%s\n", cmd);
			system(cmd);
