...
/* Zyxel_Sean: LTE3301-PLUS always get NULL from RDM_OID_USB_USBHOSTS_HOST_DEV_PAR at this point, so we change to get USB info from RDM_OID_USB_USBHOSTS_HOST_DEV */
#if 0
	objIndex_t usbIid = {0};
	usbIid.level = 3;
	usbIid.idx[0] = 1;
	...
	//MTKSOC Patches
	IID_INIT(iid);
	while(zcfgFeObjJsonGetNext(RDM_OID_USB_USBHOSTS_HOST_DEV, &iid, &usbHostDevParObj) == ZCFG_SUCCESS){
...
zcfgRet_t zcfgFeDalCardPageStatusGet(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg)
{
	...
	struct json_object *dslChannelJarray = NULL, *dslChannelObj = NULL;
	struct json_object *gponstatusObj = NULL;
	struct json_object *rountingJarray = NULL, *routingObj = NULL;
//MTKSOC Patches
	struct json_object *cellIntfObj = NULL;
...
//MTKSOC Patches
	...
	rountingJarray = json_object_new_array();
	IID_INIT(iid);
	while(zcfgFeObjJsonGetNext(RDM_OID_ROUTING_ROUTER, &iid, &routingObj) == ZCFG_SUCCESS) {
		json_object_array_add(rountingJarray, routingObj);
	}
...
//MTKSOC Patches
	...
	json_object_object_add(statusJobj, "CellIntfZyIpPassthruInfo", cellIntfZyIpPassthruObj);
	json_object_object_add(statusJobj, "RouterInfo", rountingJarray);
...
void zcfgFeDalShowNetworkMAP(struct json_object *Jarray){
	...
	printf("%-20s %-17s %-30s %-20s %-16s %-18s %-5s\n",
		    "Name", "IP Address", "IPv6 Address", "MAC Address", "Address Source", "connected time(s)", "Connection Type");
#else
...
#ifdef TELIA_CUSTOMIZATION
			json_object_get_string(json_object_object_get(obj, "X_ZYXEL_ConnectionDuration")),
#endif
