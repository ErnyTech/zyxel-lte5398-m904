...
#ifdef ZYXEL_PHONE_VOLTE_SUPPORT
	struct json_object *voipVolteObj = NULL, *voipVolteJarray = NULL;
#endif	
...
#ifdef ZYXEL_PHONE_VOLTE_SUPPORT
	/*RDM_OID_VOICE_VOLTE  Device.Services.VoiceService.i.X_ZYXEL_VOLTE   VolteEnable*/
	voipVolteJarray = json_object_new_array();
	IID_INIT(iid);
	while(zcfgFeObjJsonGetNext(RDM_OID_VOICE_VOLTE, &iid, &voipVolteObj) == ZCFG_SUCCESS) {
		Jaddi(voipVolteObj, "idx0", iid.idx[0]);
		Jaddi(voipVolteObj, "idx1", iid.idx[1]);
		Jaddi(voipVolteObj, "idx2", iid.idx[2]);
		json_object_array_add(voipVolteJarray, voipVolteObj);
	}
#endif
...
#ifdef ZYXEL_PHONE_VOLTE_SUPPORT
	json_object_object_add(getAllReqIntfObjs, "VolteInfo", voipVolteJarray);
#endif
	return ZCFG_SUCCESS;
