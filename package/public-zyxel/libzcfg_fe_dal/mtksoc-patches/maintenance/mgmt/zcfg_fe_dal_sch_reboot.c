...
#define SCHEDULE_REBOOT_TIME_LEN 20
dal_param_t SCH_REBOOT_param[]={		//RDM_OID_REMO_SRV
	//periodic reboot time format: 0001-01-01P00:00:00Z
	//one-shot reboot time format: 0001-01-01T00:00:00Z
	{"scheduleReboot", 					dalType_string,	SCHEDULE_REBOOT_TIME_LEN,	SCHEDULE_REBOOT_TIME_LEN,		NULL, NULL, 				dal_Edit},
...
int verifyScheduleRebootTimeFormat(char *ScheduleReboot, dateTime_t* dtReturn, char *replyMsg)
{
	int i=0, j=0, next=0;
	char buf[32] = {0};
	int ret = 0;
	...
	if (ScheduleReboot) {
		...
		if (strstr(ScheduleReboot, "P")) {
			dt->type = 1;
			// Do periodic, 0001-01-01P00:00:00Z
			for (i=0; i<strlen(ScheduleReboot); i++) {
				if (ScheduleReboot[i] == 'P') {
					next++;
					continue;
				}
				if (next > 0) {
					...
					buf[j++] = ScheduleReboot[i];
				}
			}
		} else if (strstr(ScheduleReboot, "T")) {
			dt->type = 0;
...
zcfgRet_t zcfgFeDal_schReboot_Get(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	...
	if(jsonType == json_type_object){
		...
		if(!strcmp(cmdType, "GUI")){
			dt = (dateTime_t*)malloc(sizeof(dateTime_t));
			result = verifyScheduleRebootTimeFormat(mgmtSrvObj->ScheduleReboot, dt, replyMsg);
			if(result != 0){
				if(replyMsg) strcpy(replyMsg, "Request reject");
				return ZCFG_REQUEST_REJECT;
			}
			...
			paramJobj = json_object_new_object();
			json_object_object_add(paramJobj, "ScheduleReboot", json_object_new_string(mgmtSrvObj->ScheduleReboot));
			json_object_array_add(Jarray, paramJobj);
		}
...
void zcfgFeDalShowSchReboot(struct json_object *Jarray){
	...
	printf("%s%s \n","ScheduleReboot:", json_object_get_string(json_object_object_get(obj, "ScheduleReboot")));
		
}


zcfgRet_t zcfgFeDal_schReboot_Edit(struct json_object *Jobj, char *replyMsg){
	zcfgRet_t ret = ZCFG_SUCCESS;
	enum json_type jsonType;
	rdm_MgmtSrv_t *mgmtSrvObj = NULL;
	objIndex_t objIid;
	char *scheduleReboot = NULL;
	const char* cmdType = NULL;
	int result = 0;
	...
	if(jsonType == json_type_object){
		...
		if(!strcmp(cmdType, "GUI")){
			scheduleReboot = (char*)malloc(sizeof(char)*(SCHEDULE_REBOOT_TIME_LEN + 1));
			snprintf(scheduleReboot, (SCHEDULE_REBOOT_TIME_LEN + 1), "%4d-%02d-%02d%s%02d:%02d:%02dZ",
			...
			scheduleReboot = Jgets(Jobj, "scheduleReboot");
		}
		result = verifyScheduleRebootTimeFormat(scheduleReboot, NULL, replyMsg);
		...
		strncpy(mgmtSrvObj->ScheduleReboot,scheduleReboot, sizeof(mgmtSrvObj->ScheduleReboot));
...
zcfgRet_t zcfgFeDalSchReboot(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	zcfgRet_t ret = ZCFG_SUCCESS;
	if(!strcmp(method, "PUT"))
		ret = zcfgFeDal_schReboot_Edit(Jobj, replyMsg);
	else if(!strcmp(method, "GET"))
		ret = zcfgFeDal_schReboot_Get(Jobj, Jarray, NULL);
	else
