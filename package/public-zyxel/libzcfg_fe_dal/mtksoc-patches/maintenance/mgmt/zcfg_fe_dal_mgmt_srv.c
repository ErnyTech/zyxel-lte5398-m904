...
#define DNA_HTTPS_PORT "20443"
#define DNA_SSH_PORT "20022"
#endif
...
zcfgRet_t zcfgFeDal_mgmtSrv_Get(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	...
	if(!strcmp(cmdType, "GUI")){
		...
		json_object_array_add(conflictPortJarray, JSON_OBJ_COPY(json_object_new_string(DNA_HTTPS_PORT)));
		json_object_array_add(conflictPortJarray, JSON_OBJ_COPY(json_object_new_string(DNA_SSH_PORT)));
	}
#endif
	...
	while(zcfgFeObjJsonGetNextWithoutUpdate(RDM_OID_REMO_SRV, &iid, &obj) == ZCFG_SUCCESS) {
		...
		//Telia show to admin configurable telnet option (preferably on a separate page)
		//if(0 == strcmp(loginLevel, "medium")){
			if(!strcmp(Name, "TELNET")){
				#if 1
				json_object_array_add(conflictPortJarray, JSON_OBJ_COPY(json_object_object_get(obj, "Port")));
				#else
				pramJobj = json_object_new_object();
				json_object_object_add(pramJobj, "Index", json_object_new_int(iid.idx[0]));
				json_object_object_add(pramJobj, "Name", JSON_OBJ_COPY(json_object_object_get(obj, "Name")));
		...
		//}
		#elif defined(DNA_CUSTOMIZATION)
		Name = json_object_get_string(json_object_object_get(obj, "Name"));

		//hide HTTPs, SSH, FTP services in DNA's WebGUI
		if(!strcmp(cmdType, "GUI") && (!strcmp(Name, "HTTPS") || !strcmp(Name, "SSH") || !strcmp(Name, "FTP"))){
			zcfgFeJsonObjFree(obj);
			continue;
		}
		else{
...
#ifdef DNA_CUSTOMIZATION
	if(!strcmp(cmdType, "GUI")){
		json_object_array_add(Jarray, conflictPortJarray);
	}
#endif
	return ret;
}
