...
/* set CellWanBackup object */
{"CellWanBackup_WanBackupEnable", dalType_boolean, 0, 0, NULL},
{"CellWanBackup_ETHIPInterface", dalType_string, 0, 0, NULL},
{"CellWanBackup_CellIPInterface", dalType_string, 0, 0, NULL},
{"CellWanBackup_PrimaryWANLinkType", dalType_string, 0, 0, NULL},
{"CellWanBackup_ConnectionCheckSiteUser", dalType_string, 0, 0, NULL},
{"CellWanBackup_ConnectionCheckSiteID", dalType_int, 0, 0, NULL},
{"CellWanBackup_Interval", dalType_int, 0, 0, NULL},
{"CellWanBackup_FailLimit", dalType_int, 0, 0, NULL},
...
zcfgRet_t setCellularCpeObj(struct json_object *Jobj)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
//	struct json_object *cellularCPEWanBackupConfigObj = NULL;
	rdm_ZyWWANCellularCpeWanBackupConfig_t	*cellularCPEWanBackupConfigObj = NULL;
	objIndex_t cellularCPEWanBackupConfigIid = {0};
	bool enable = false;
	int interval = 0;
	int failLimit = 0;
	bool pingGw = false;
	...
	bool bWanBackupEnable = true;
	const char *cETHIPInterface = NULL;
	const char *cCellIPInterface = NULL;
	const char *cPrimaryWANLinkType = NULL;
	const char *cConnectionCheckSiteUser = NULL;
	int nConnectionCheckSiteID = 0;
	int nInterval = 10;
	int nFailLimit = 3;
	
	IID_INIT(cellularCPEWanBackupConfigIid);

	ret = zcfgFeObjStructGet(RDM_OID_ZY_W_W_A_N_CELLULAR_CPE_WAN_BACKUP_CONFIG, &cellularCPEWanBackupConfigIid, (void **)&cellularCPEWanBackupConfigObj);
	if(ret == ZCFG_SUCCESS)
	{
		bWanBackupEnable		 = json_object_get_boolean(json_object_object_get(Jobj, "CellWanBackup_WanBackupEnable"       ));
		cETHIPInterface          = json_object_get_string(json_object_object_get(Jobj, "CellWanBackup_ETHIPInterface"         ));
		cCellIPInterface         = json_object_get_string(json_object_object_get(Jobj, "CellWanBackup_CellIPInterface"        ));
		cPrimaryWANLinkType      = json_object_get_string(json_object_object_get(Jobj, "CellWanBackup_PrimaryWANLinkType"     ));
		cConnectionCheckSiteUser = json_object_get_string(json_object_object_get(Jobj, "CellWanBackup_ConnectionCheckSiteUser"));
	    nConnectionCheckSiteID   = json_object_get_int   (json_object_object_get(Jobj, "CellWanBackup_ConnectionCheckSiteID"  ));
	    nInterval                = json_object_get_int   (json_object_object_get(Jobj, "CellWanBackup_Interval"               ));
	    nFailLimit               = json_object_get_int   (json_object_object_get(Jobj, "CellWanBackup_FailLimit"              ));

		cellularCPEWanBackupConfigObj->WanBackupEnable = bWanBackupEnable;

		strcpy(cellularCPEWanBackupConfigObj->ETHIPInterface, cETHIPInterface					);
		strcpy(cellularCPEWanBackupConfigObj->CellIPInterface, cCellIPInterface					);
		strcpy(cellularCPEWanBackupConfigObj->PrimaryWANLinkType, cPrimaryWANLinkType			);
		strcpy(cellularCPEWanBackupConfigObj->ConnectionCheckSiteUser, cConnectionCheckSiteUser	);

		cellularCPEWanBackupConfigObj->ConnectionCheckSiteID = nConnectionCheckSiteID;
		cellularCPEWanBackupConfigObj->Interval = nInterval;
		cellularCPEWanBackupConfigObj->FailLimit = nFailLimit;

		zcfgFeObjStructSet(RDM_OID_ZY_W_W_A_N_CELLULAR_CPE_WAN_BACKUP_CONFIG, &cellularCPEWanBackupConfigIid, (void *)cellularCPEWanBackupConfigObj, NULL);
		zcfgFeObjStructFree(cellularCPEWanBackupConfigObj);

		system("killall -9 wwanbackupd 2>/dev/null");
		system("wwanbackupd 0 1&");
	}
	
/*
	ret = zcfgFeObjJsonGetWithoutUpdate(RDM_OID_ZY_W_W_A_N_CELLULAR_CPE_WAN_BACKUP_CONFIG, &cellularCpeIid, &cellularCpeObj);
	...
	if(ret == ZCFG_SUCCESS)
	{
		...
		if(json_object_object_get(Jobj, "CellWanBackup_ETHIPInterface"))
			
		json_object_object_add(cellularCpeObj, "ETHIPInterface", json_object_new_string(destination));
		if(json_object_object_get(Jobj, "CellWanBackup_CellIPInterface"))
		json_object_object_add(cellularCpeObj, "CellIPInterface", json_object_new_string(destination));
		if(json_object_object_get(Jobj, "CellWanBackup_PrimaryWANLinkType"))
		json_object_object_add(cellularCpeObj, "PrimaryWANLinkType", json_object_new_string(destination));
		if(json_object_object_get(Jobj, "CellWanBackup_ConnectionCheckSiteUser"))
		json_object_object_add(cellularCpeObj, "ConnectionCheckSiteUser", json_object_new_string(destination));

		if(json_object_object_get(Jobj, "CellWanBackup_ConnectionCheckSiteID"))
		json_object_object_add(cellularCpeObj, "ConnectionCheckSiteID", json_object_new_int(failLimit));
		if(json_object_object_get(Jobj, "CellWanBackup_Interval"))
		json_object_object_add(cellularCpeObj, "Interval", json_object_new_int(failLimit));
		if(json_object_object_get(Jobj, "CellWanBackup_FailLimit"))
		json_object_object_add(cellularCpeObj, "FailLimit", json_object_new_int(failLimit));

		ret = zcfgFeObjJsonSetWithoutApply(RDM_OID_ZY_W_W_A_N_CELLULAR_CPE_WAN_BACKUP_CONFIG, &cellularCpeIid, cellularCpeObj, NULL);
	}

	zcfgFeJsonObjFree(cellularCpeObj);
*/
	return ret;
}
...
zcfgRet_t zcfgFeDal_WwanBackup_Edit(struct json_object *Jobj, char *replyMsg){
	...
	char pppIface[32] = {0};
	bool resetNow = false;
	bool isCellularCpeDevice = false;
	...
	if(resetNow == false)
	{
		isCellularCpeDevice = json_object_get_boolean(json_object_object_get(Jobj, "isCellular_CPE_Device"));
		if (isCellularCpeDevice)
		{
			setCellularCpeObj(Jobj);
		}
		else
		{	
...
zcfgRet_t zcfgFeDal_WwanBackup_Get(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	zcfgRet_t ret = ZCFG_SUCCESS;
	struct json_object *paramJobj = NULL, *ipIfaceV4Obj = NULL, *dnsObj = NULL, *pppIfaceObj = NULL, *pingObj = NULL, *cellularCpeObj = NULL, *emailObj = NULL, *budgetObj = NULL;
	struct json_object *wwanBackupObj = NULL;
	objIndex_t wwanBackupIid = {0}, ipIfaceV4Iid = {0}, dnsIid = {0}, pppIfaceIid = {0}, pingIid = {0}, emailIid = {0}, budgetIid = {0}, cellularCpeIid = {0};
	...
	if(zcfgFeObjJsonGetWithoutUpdate(RDM_OID_ZY_W_W_A_N_BACKUP, &wwanBackupIid, &wwanBackupObj) == ZCFG_SUCCESS){
		...
		json_object_object_add(paramJobj, "isCellular_CPE_Device", JSON_OBJ_COPY(json_object_object_get(wwanBackupObj, "isCellular_CPE_Device")));
	...
	if(zcfgFeObjJsonGetWithoutUpdate(RDM_OID_ZY_W_W_A_N_CELLULAR_CPE_WAN_BACKUP_CONFIG, &cellularCpeIid, &cellularCpeObj) == ZCFG_SUCCESS)
	{
		json_object_object_add(paramJobj, "CellWanBackup_WanBackupEnable",			JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "WanBackupEnable")));
		json_object_object_add(paramJobj, "CellWanBackup_ETHLinkInterface",			JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "ETHLinkInterface")));
		json_object_object_add(paramJobj, "CellWanBackup_CellLinkInterface",		JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "CellLinkInterface")));
		json_object_object_add(paramJobj, "CellWanBackup_ETHIPInterface",			JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "ETHIPInterface")));
		json_object_object_add(paramJobj, "CellWanBackup_CellIPInterface",			JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "CellIPInterface")));
		json_object_object_add(paramJobj, "CellWanBackup_PrimaryWANLinkType",		JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "PrimaryWANLinkType")));
		json_object_object_add(paramJobj, "CellWanBackup_ConnectionCheckSite1_Name",JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "ConnectionCheckSite1_Name")));
		json_object_object_add(paramJobj, "CellWanBackup_ConnectionCheckSite1_IP",	JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "ConnectionCheckSite1_IP")));
		json_object_object_add(paramJobj, "CellWanBackup_ConnectionCheckSite2_Name",JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "ConnectionCheckSite2_Name")));
		json_object_object_add(paramJobj, "CellWanBackup_ConnectionCheckSite2_IP",	JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "ConnectionCheckSite2_IP")));
		json_object_object_add(paramJobj, "CellWanBackup_ConnectionCheckSite3_Name",JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "ConnectionCheckSite3_Name")));
		json_object_object_add(paramJobj, "CellWanBackup_ConnectionCheckSite3_IP",	JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "ConnectionCheckSite3_IP")));
		json_object_object_add(paramJobj, "CellWanBackup_ConnectionCheckSiteUser",	JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "ConnectionCheckSiteUser")));
		json_object_object_add(paramJobj, "CellWanBackup_ConnectionCheckSiteID",	JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "ConnectionCheckSiteID")));
		json_object_object_add(paramJobj, "CellWanBackup_Interval",					JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "Interval")));
		json_object_object_add(paramJobj, "CellWanBackup_FailLimit",				JSON_OBJ_COPY(json_object_object_get(cellularCpeObj, "FailLimit")));
		zcfgFeJsonObjFree(cellularCpeObj);
	}
