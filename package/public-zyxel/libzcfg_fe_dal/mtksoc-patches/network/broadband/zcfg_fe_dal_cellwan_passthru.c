...
#else // apply to all APN entries
			while (zcfgFeObjJsonGetNext(RDM_OID_CELL_INTF_ZY_IP_PASS_THRU, &intfZyIpPassthruIid, &intfZyIpPassthruObj) == ZCFG_SUCCESS) {

				if(json_object_object_get(Jobj, "Passthru_Enable")) json_object_object_add(intfZyIpPassthruObj, "Enable", json_object_new_boolean(passthruEnable));
				if(json_object_object_get(Jobj, "Passthru_Mode")) json_object_object_add(intfZyIpPassthruObj, "ConnectionMode", json_object_new_string(passthruMode));
				if(json_object_object_get(Jobj, "Passthru_MacAddr")) json_object_object_add(intfZyIpPassthruObj, "MACAddress", json_object_new_string(passthruMacAddr));
...
#ifdef ZYXEL_CELLWAN_WITH_VLAN_INTF_GROUP
		printf("%-6s %-12s %-16s %-20s %-40s\n", "Index", "Name", "Enable/Disable", "Connection Mode", "Passthrough to fixed MAC");
		printf("%-6s %-12s %-16s %-20s %-40s\n", "=====", "====", "==============", "===============", "========================");
