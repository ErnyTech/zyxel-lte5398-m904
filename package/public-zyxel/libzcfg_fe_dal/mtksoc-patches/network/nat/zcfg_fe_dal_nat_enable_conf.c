...
zcfgRet_t zcfgFeDalNatEnableConfGet(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	...
	objIid.level = 1;
	objIid.idx[0] = 1;
	if( zcfgFeObjJsonGet(RDM_OID_NAT_INTF_SETTING, &objIid, &natEnableJobj) == ZCFG_SUCCESS)
#else
...
zcfgRet_t zcfgFeDalNatEnableConfEdit(struct json_object *Jobj, char *replyMsg)
{
	...
	objIid.level = 1;
	objIid.idx[0] = 1;
	if (zcfgFeObjJsonGet(RDM_OID_NAT_INTF_SETTING, &objIid, &natIntfObj) == ZCFG_SUCCESS)
#else
...
#else
				//char nat_interface[64] = {0};
				//snprintf(nat_interface, sizeof(nat_interface), "IP.Interface.2");
				json_object_object_add(natIntfObj, "Interface", json_object_new_string("IP.Interface.2"));
#endif
