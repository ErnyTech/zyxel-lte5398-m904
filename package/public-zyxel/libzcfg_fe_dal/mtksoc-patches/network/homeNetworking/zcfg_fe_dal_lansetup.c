...
dal_param_t LANSETUP_param[]={
	...
	//Add for ISP show wan DNS
	{"DNS_Servers3", dalType_v4Addr,	0,	0,	NULL},
	{"DNS_Servers4", dalType_v4Addr,	0,	0,	NULL},
#endif
	//LAN IPv6 Mode Setup
...
#endif
#ifdef ZyXEL_DHCP_OPTION82
	{"Option_82_Enable", dalType_boolean,	0,	0,	NULL},
	{"CircuitID", dalType_string,	0,	0,	NULL},
	{"RemoteID", dalType_string,	0,	0,	NULL},
#endif	
	{NULL,					0,	0,	0,	NULL},
};
...
zcfgRet_t setDHCPState(struct json_object *Jobj){
	...
	bool option82Enable = false, curoption82Enable = false;
	const char *curcircuitID = NULL, *circuitID = NULL;
	const char *curremoteID = NULL, *remoteID = NULL;
...
#ifdef ZyXEL_DHCP_OPTION82			
			curoption82Enable = json_object_get_boolean(json_object_object_get(dhcpRelayObj, "Option_82_Enable"));
			curcircuitID = json_object_get_string(json_object_object_get(dhcpRelayObj, "CircuitID"));
			curremoteID = json_object_get_string(json_object_object_get(dhcpRelayObj, "RemoteID"));
#endif			
			break;
		}
	...
	if(enableDhcp && !strcmp(DHCPType, "DHCPRelay")){
...
#ifdef ZyXEL_DHCP_OPTION82		
		option82Enable = json_object_get_boolean(json_object_object_get(Jobj, "Option_82_Enable"));
		circuitID = json_object_get_string(json_object_object_get(Jobj, "CircuitID"));
		remoteID = json_object_get_string(json_object_object_get(Jobj, "RemoteID"));
		
		if(option82Enable == true ) {
			if (circuitID == NULL ) {
				if(curcircuitID == NULL || curcircuitID[0] == '\0')
				printf("\nInvalid parameter: Circuit ID\n");
				return ZCFG_INVALID_PARAM_VALUE;
			}
			if (remoteID == NULL ) {
				if(curremoteID == NULL || curremoteID[0] == '\0')
				printf("\nInvalid parameter: Circuit ID\n");
				return ZCFG_INVALID_PARAM_VALUE;
			}			
		}	
		...
		if(curRelayDhcp){
#ifdef ZyXEL_DHCP_OPTION82				
			if(strcmp(dhcpRelayAddr, curdhcpRelayAddr) || strcmp(circuitID, curcircuitID) || strcmp(remoteID, curremoteID) || (option82Enable != curoption82Enable)){
#else
...
#ifdef ZyXEL_DHCP_OPTION82				
			json_object_object_add(dhcpRelayObj, "Option_82_Enable", json_object_new_boolean(option82Enable));
			json_object_object_add(dhcpRelayObj, "CircuitID", json_object_new_string(circuitID));
			json_object_object_add(dhcpRelayObj, "RemoteID", json_object_new_string(remoteID));
#endif			
...
#ifdef TELIA_CUSTOMIZATION
	struct json_object *wandhcpv4srvObj = NULL;
	objIndex_t wandhcpv4srvIid = {0};
	const char *wandnsserver = NULL;
	char  *dns3 = NULL, *dns4 = NULL;
#endif
		...
		if(!strncmp(json_object_get_string(json_object_object_get(ipifaceObj, "X_ZYXEL_IfName")), "br", 2)){
...
#ifdef ZyXEL_DHCP_OPTION82						
					if(json_object_get_boolean(json_object_object_get(dhcpv4fwdObj, "Option_82_Enable")))
						json_object_object_add(paramJobj, "Option_82_Enable", json_object_new_boolean(true));
					else
						json_object_object_add(paramJobj, "Option_82_Enable", json_object_new_boolean(false));
					json_object_object_add(paramJobj, "CircuitID", JSON_OBJ_COPY(json_object_object_get(dhcpv4fwdObj, "CircuitID")));
					json_object_object_add(paramJobj, "RemoteID", JSON_OBJ_COPY(json_object_object_get(dhcpv4fwdObj, "RemoteID")));
#endif					
					dchprelay = true;
			...
			while(zcfgFeObjJsonGetNext(RDM_OID_DHCPV4_SRV_POOL, &dhcpv4srvIid, &dhcpv4srvObj) == ZCFG_SUCCESS){
				if(!strcmp(json_object_get_string(json_object_object_get(dhcpv4srvObj, "Interface")),currintf) && !dchprelay){
					...
					while(zcfgFeObjJsonGetNext(RDM_OID_DNS_CLIENT_SRV, &wandhcpv4srvIid, &wandhcpv4srvObj) == ZCFG_SUCCESS){
						wandnsserver = json_object_get_string(json_object_object_get(wandhcpv4srvObj, "DNSServer"));
						if(wandnsserver != NULL){
							strcpy(buff, wandnsserver);
							dns3 = strtok_r(buff, ",", &dns4);
							json_object_object_add(paramJobj, "DNS_Servers3", json_object_new_string(dns3));
								if(dns4 != NULL){
									json_object_object_add(paramJobj, "DNS_Servers4", json_object_new_string(dns4));
								}
						}
					}
					zcfgFeJsonObjFree(wandhcpv4srvObj);
#endif
