...
#ifdef ZYXEL_CHANGE_WIFI_COUNTRY_FOR_RUSSIA
	{"countryCode",				dalType_string,		0,	0,	NULL,		NULL,			0},//MTKSOC patches
#endif
...
#ifdef ZYXEL_CHANGE_WIFI_COUNTRY_FOR_RUSSIA
			json_object_object_add(pramJobj, "countryCode", JSON_OBJ_COPY(json_object_object_get(obj2, "X_ZYXEL_CountryCode")));//MTKSOC patches
#endif
			{
...
#ifdef ZYXEL_CHANGE_WIFI_COUNTRY_FOR_RUSSIA
	if(isCountryChanged == false){
		if(wifiRadioObj) zcfgFeObjJsonSetWithoutApply(RDM_OID_WIFI_RADIO, &wifiRadioIid, wifiRadioObj, NULL);
	}
	else{
		if(wifiRadioObj) zcfgFeObjJsonBlockedSet(RDM_OID_WIFI_RADIO, &wifiRadioIid, wifiRadioObj, NULL);
	}
#else
	if(wifiRadioObj) zcfgFeObjJsonSetWithoutApply(RDM_OID_WIFI_RADIO, &wifiRadioIid, wifiRadioObj, NULL);
#endif
...
#ifdef ZYXEL_CHANGE_WIFI_COUNTRY_FOR_RUSSIA
	countryCode = json_object_get_string(json_object_object_get(Jobj, "countryCode"));//MTKSOC patches
#endif	
	//WiFi Access Point
...
#ifdef ZYXEL_CHANGE_WIFI_COUNTRY_FOR_RUSSIA
	json_object_object_add(wifiRadioObj, "X_ZYXEL_CountryCode", json_object_new_string(countryCode));
#endif
	return ret;
}
...
void prepareWifiRadioObj(struct json_object *Jobj){
	...
	//MTKSOC patches: Change country code function for Russia
#ifdef ZYXEL_CHANGE_WIFI_COUNTRY_FOR_RUSSIA
	if(!json_object_object_get(Jobj, "countryCode")){
			countryCode = json_object_get_string(json_object_object_get(wifiRadioObj, "X_ZYXEL_CountryCode"));
		json_object_object_add(Jobj, "countryCode", json_object_new_string(countryCode));
	}

	if(!strcmp(countryCode,json_object_get_string(json_object_object_get(wifiRadioObj, "X_ZYXEL_CountryCode"))))
