...
zcfgRet_t zcfgFeDalDefaultSsidGet(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	...
	char *tempSSIDPrefix = json_object_get_string(json_object_object_get(wifissidObj, "X_ZYXEL_SSIDPrefix"));
 	
    zyUtilIGetBaseMAC(macAddress);
    strncpy(tempMAC, macAddress + 8, 4);
...
#else
	printf("%s%s \n","Default-2.4G-SSID:", json_object_get_string(json_object_object_get(obj, "DefaultSSID")));
	printf("%s%s_5G \n","Default-5G-SSID:", json_object_get_string(json_object_object_get(obj, "DefaultSSID")));
#endif
