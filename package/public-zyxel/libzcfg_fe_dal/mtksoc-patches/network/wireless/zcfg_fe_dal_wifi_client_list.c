...
void zcfgFeDalShowWifiClientList(struct json_object *Jarray){						
	...
	snprintf(sysCmd, sizeof(sysCmd), "iwpriv rai0 show zystainfo");
	system(sysCmd);
#endif
	sleep(2);
#ifdef TELIA_CUSTOMIZATION
	printf("WIFI 2.4G Client:\n");
#endif
...
#ifdef TELIA_CUSTOMIZATION
	printf("WIFI 5G Client:\n");
	FILE *ffp = fopen("/tmp/ecn_assoc_list_5g", "r");
	if(ffp) {
		while(fgets(cmd, sizeof(cmd)-1, ffp) != NULL) { 
			printf("%s", cmd);
			memset(cmd, 0, sizeof(cmd));
		}
		pclose(ffp);
