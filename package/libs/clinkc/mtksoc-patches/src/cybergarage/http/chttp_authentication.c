...
#ifdef JESSIE_DBG
		fprintf(fp,"Jessie != CG_HTTP_STATUS_UNAUTHORIZED,exit\n");
		fclose(fp);
#endif
		return httpRes;
	}
...
#ifdef JESSIE_DBG
			fprintf(fp,"httpRes = AUTH_WRONG_INPUT\n");
#endif
		}
#ifdef JESSIE_DBG
		fclose(fp);
...
#ifdef JESSIE_DBG
			fprintf(fp,"status code == CG_HTTP_STATUS_UNAUTHIRUZED (");
			fflush(fp);
#endif
...
#ifdef JESSIE_DBG
		fprintf(fp,"I don't know why, but it is AUTH_UNSUPPORT_AUTH\n\n");
		fflush(fp);
		fclose(fp);
#endif
...
void sendDigestChallenge(CgHttpRequest *httpReq, char *realm)
{
	...
	generateNonce(nonce);
#ifdef JESSIE_DBG
	fprintf(fp,"%s() Jessie generateNonce\n", __FUNCTION__);//Jessie
	fflush(fp);
#endif
	strcpy(newEntry->nonce, nonce);
#ifdef JESSIE_DBG
	printf("%s() Jessie newEntry->nonce=%s, nonce=%s\n", __FUNCTION__, newEntry->nonce, nonce);//Jessie
#endif
	if(realm == NULL)
	...
	while(now != NULL){
#ifdef JESSIE_DBG
		fprintf(fp,"%s() Jessie now != NULL\n", __FUNCTION__);//Jessie
#endif
		if(!strcmp(now->clientHost, newEntry->clientHost)){
#ifdef JESSIE_DBG
			fprintf(fp,"%s() Jessie compare now clientHost to new clientHost, found!!!\n", __FUNCTION__);//Jessie
			fflush(fp);
#endif
			break;
		}
	...
	if(digestHead == NULL){
#ifdef JESSIE_DBG
		fprintf(fp,"%s() Jessie digestHead==NULL\n", __FUNCTION__);//Jessie
		fflush(fp);
#endif
		digestHead = newEntry;
		digestTail = newEntry;
...
#ifdef JESSIE_DBG
		fprintf(fp,"%s() Jessie now!=NULL, do digect replace entry\n", __FUNCTION__);//Jessie
		fflush(fp);
#endif
...
#ifdef JESSIE_DBG
	fprintf(fp,"\n%s() Jessie printf httpRes = %d\n", __FUNCTION__, CG_HTTP_STATUS_UNAUTHORIZED);
	fflush(fp);
#endif
...
#ifdef JESSIE_DBG
	fprintf(fp,"%s() Jessie End authHeader = (%s)\n", __FUNCTION__,authHeader);//Jessie
	fflush(fp);
#endif
...
authRet_t cg_http_request_check_digest_auth(CgHttpRequest *httpReq, char *user, char *passwd)
{
...
#ifdef JESSIE_DBG
	printf("%s() Jessie auth->clientHost=%s\n", __FUNCTION__, auth->clientHost);//Jessie
#endif
	/*get authentication header line*/
	headerLine = cg_http_request_getauth(httpReq);
	if (headerLine == NULL){
#ifdef JESSIE_DBG
		fprintf(fp,"%s() headerLine == NULL\n", __FUNCTION__);//Jessie
#endif
	...
	if (strcmp(authMethod, "Digest") == 0){  /*digest authentication*/
		...
		if(!auth->realm){
			printf("error: can not get realm");
#ifdef JESSIE_DBG
			fprintf(fp,"error: can not get realm\n");
			fclose(fp);
#endif
			ZOS_FREE(auth);
		...
		if(!auth->nonce){
			printf("error: can not get nonce");
#ifdef JESSIE_DBG
			fprintf(fp,"error: can not get nonce");
			fclose(fp);
#endif
			ZOS_FREE(auth);
			return HTTP_AUTH_ERROR;
...
#ifdef JESSIE_DBG
			fprintf(fp,"error: can not get response\n");
			fclose(fp);
#endif
			ZOS_FREE(auth);
			return HTTP_AUTH_ERROR;
...
#ifdef JESSIE_DBG
			fprintf(fp,"error: can not get username\n");
			fclose(fp);
#endif
			ZOS_FREE(auth);
...
#ifdef JESSIE_DBG
			fprintf(fp,"error: username check error\n");
			fclose(fp);
#endif
			ZOS_FREE(auth);
			return HTTP_AUTH_INVALID_USERNAME;
...
#ifdef JESSIE_DBG
					fprintf(fp,"%s() Jessie realm is not the same!!!!\n", __FUNCTION__);//Jessie
					fclose(fp);
#endif
					ZOS_FREE(auth);
					cg_http_server_unlock(httpServer);
...
//					printf("%s() Jessie nonce is not the same, stale is TRUE!!!!\n", __FUNCTION__);//Jessie
					stale = TRUE;
				}
...
#ifdef JESSIE_DBG
					fprintf(fp,"%s() Jessie qop is not the same\n", __FUNCTION__);//Jessie
					fclose(fp);
#endif
					ZOS_FREE(auth);
					cg_http_server_unlock(httpServer);
...
#ifdef JESSIE_DBG
					fprintf(fp,"%s() Jessie algorithm is not the same\n", __FUNCTION__);//Jessie
					fclose(fp);
#endif
					ZOS_FREE(auth);
					cg_http_server_unlock(httpServer);
...
#ifdef JESSIE_DBG
				fprintf(fp,"error: can not get opaque\n");
				fclose(fp);
#endif
				ZOS_FREE(auth);
				cg_http_server_unlock(httpServer);
...
#ifdef JESSIE_DBG
					fprintf(fp,"%s() Jessie opaque is not the same\n", __FUNCTION__);//Jessie
					fclose(fp);
#endif
					ZOS_FREE(auth);
					cg_http_server_unlock(httpServer);
...
*/
		if (!(auth->qop)) {
			if((auth->algorithm)&&(strcmp(auth->algorithm, "MD5") != 0)){
#ifdef JESSIE_DBG
				fprintf(fp,"%s() Jessie AUTH_UNSUPPORT_ALGORITHM \n", __FUNCTION__);//Jessie
				fclose(fp);
#endif
				auth->result = AUTH_UNSUPPORT_ALGORITHM;
				ZOS_FREE(auth);
			...
			if((auth->algorithm)&&(strcmp(auth->algorithm, "MD5") != 0)&&(strcmp(auth->algorithm,"MD5-sess") != 0)){
#ifdef JESSIE_DBG
				fprintf(fp,"%s() Jessie 5555 AUTH_UNSUPPORT_ALGORITHM\n", __FUNCTION__);//Jessie
#endif
				auth->result = AUTH_UNSUPPORT_ALGORITHM;
				ZOS_FREE(auth);
#ifdef JESSIE_DBG
...
#ifdef JESSIE_DBG
			fprintf(fp,"%s() JessieAAA digest is the same as auth->response, HTTP_AUTH_SUCCESS!!!\n", __FUNCTION__);//Jessie
			fclose(fp);
#endif
			ZOS_FREE(auth);
			cg_http_server_unlock(httpServer);
...
#ifdef JESSIE_DBG
			fprintf(fp,"%s() JessieAAA digest is not the same as auth->response HTTP_AUTH_INVALID_PASSWORD\n", __FUNCTION__);//Jessie
			fclose(fp);
#endif
			ZOS_FREE(auth);
			cg_http_server_unlock(httpServer);
...
#ifdef JESSIE_DBG
		fprintf(fp,"invalid authenticcation method\n");
		fclose(fp);
#endif
		ZOS_FREE(auth);
		return HTTP_AUTH_NOT_EXIST;
...
#ifdef JESSIE_DBG
	fprintf(fp,"Jessie HTTP_AUTH_NOT_EXIST\n ");
	fclose(fp);
#endif
	ZOS_FREE(auth);
	return HTTP_AUTH_NOT_EXIST;
...
void sendBasicChallenge(CgHttpRequest *httpReq, char *realm)
{
	...
	httpRes = cg_http_response_new();
#ifdef JESSIE_DBG
	fprintf(fp,"%s() Jessie printf httpRes = %d\n\n\n", __FUNCTION__, CG_HTTP_STATUS_UNAUTHORIZED);
	fclose(fp);
#endif
	cg_http_response_setstatuscode(httpRes, CG_HTTP_STATUS_UNAUTHORIZED);
	cg_http_response_setcontentlength(httpRes, 0);
...
authRet_t cg_http_request_check_basic_auth(CgHttpRequest *httpReq, char *user, char *passwd, BOOL cookieAuthentication)
{
	...
	if (!cookieAuthentication){
		...
		if (headerLine == NULL){
#ifdef JESSIE_DBG
			fprintf(fp,"%s() Jessie HTTP_basic_AUTH_ERROR \n");
			fclose(fp);
#endif
			return HTTP_AUTH_ERROR;
		}
		...
		if(cookieLen != 0){
			...
			if(cookie == NULL){
				printf("error: out of memory,%s,%d\n",__FUNCTION__,__LINE__);
#ifdef JESSIE_DBG
				fprintf(fp,"error: out of memory,%s,%d\n",__FUNCTION__,__LINE__);
				fclose(fp);
#endif
				return HTTP_AUTH_ERROR;
			}
...
#ifdef JESSIE_DBG
				fprintf(fp,"invalid authenticcation\n");
				fclose(fp);
#endif
				ZOS_FREE(cookie);
				return HTTP_AUTH_ERROR;
			...
			if(passphrase){
				...
				if(tmpPtr == decodePhrase || decodeUser == NULL || strcmp(decodeUser, user) != 0){
					...
					ZOS_FREE(cookie);
#ifdef JESSIE_DBG
					fprintf(fp,"Jessie HTTP_AUTH_INVALID_USERNAME\n");
					fclose(fp);
#endif
					return HTTP_AUTH_INVALID_USERNAME;
				}
					...
					ZOS_FREE(cookie);
#ifdef JESSIE_DBG
					fprintf(fp,"Jessie AUTH_INVALID_PASSWORD\n");
					fclose(fp);
#endif
					return HTTP_AUTH_INVALID_PASSWORD;
				}
...
#ifdef JESSIE_DBG
	fprintf(fp,"Jessie AUTH_INVALID_PASSWORD222222\n");
	fclose(fp);
#endif
	return HTTP_AUTH_INVALID_PASSWORD;
}
