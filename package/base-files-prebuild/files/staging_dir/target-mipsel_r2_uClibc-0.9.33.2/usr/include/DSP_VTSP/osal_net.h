...
#ifdef OSAL_NET_ENABLE_TCP_KEEPALIVE
#include <linux/tcp.h>
#endif
...
#define OSAL_NET_IPV6_INET_LINE_SZ (54)           /* Line size in if_inet6 */
#define OSAL_NET_IPV6_INET_IP_SZ   (32)           /* IPv6 size in if_inet6 */
#define OSAL_NET_INFC_MAX_SZ       (20)           /* The max size of interface name */
#define OSAL_NET_IPV6_LOCAL_PREFIX "fe80"         /* The prefix of IPv6 link local */
#define OSAL_NET_IPV6_MAC_INSERT   "fffe"         /* The IPv6 modified EUI-64 */
#define OSAL_NET_MAC_ADDR_STR_MAX  (17)           /* The max size of mac address (w/o :) */
...
typedef enum {
    ...
    OSAL_NET_SOCK_KEEPALIVE     = 6, /* Keepalive the socket as inactivity lead to RST of TCP */
} OSAL_NetSockopt;
...
OSAL_Boolean OSAL_netIsSocketIdValid(
    OSAL_NetSockId *socket_ptr);
