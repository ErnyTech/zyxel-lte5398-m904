...
typedef enum {
    ...
    OSAL_IPSEC_AUTH_ALG_LAST = OSAL_IPSEC_AUTH_ALG_HMAC_SHA1_96,
} OSAL_IpsecAuthAlg;
...
typedef struct {
    ...
    uint8                keyAh[OSAL_IPSEC_KEY_LENGTH_MAX];  /* Key for AH, HEX value */
    uint8                keyEsp[OSAL_IPSEC_KEY_LENGTH_MAX]; /* Key for ESP, HEX value */
} OSAL_IpsecSa;
