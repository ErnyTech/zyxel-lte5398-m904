...
int skb_mark_queue_mapping(unsigned int mark)
{
	unsigned int mapping_mark;
	//printk( "SKBMARK_Q_M=0x%x!!\n", SKBMARK_Q_M);
	...
	mapping_mark = mark&SKBMARK_Q_M;
	mapping_mark = mapping_mark>>SKBMARK_Q_S;
	mapping_mark = mapping_mark;
	//}

	//printk( "mapping_mark=0x%x!!\n", mapping_mark);
	return mapping_mark;
}
...
inline int rt2880_eth_send(struct net_device* dev, struct sk_buff *skb, int gmac_no)
{
  ...
  if (ack_packt==1){
	...
	mapping_mark = skb_mark_queue_mapping(skb->mark);
	cpu_ptr->txd_info3.QID = M2Q_table[mapping_mark];
#ifdef CONFIG_PSEUDO_SUPPORT
	...
	if(dbg==1){
		mapping_mark = skb_mark_queue_mapping(skb->mark);
		printk("M2Q_table[%d]=%d\n", mapping_mark, M2Q_table[mapping_mark]);
