...
uint16_t remove_vlan_tag(struct sk_buff **pskb)
{
	struct ethhdr *eth;
	struct vlan_ethhdr *veth;
	u16 vir_if_idx;
...
static inline int __skb_cow_1(struct sk_buff *skb, unsigned int headroom,
			    int cloned)
{
	int delta = 0;
...
static inline int skb_cow_head_1(struct sk_buff *skb, unsigned int headroom)
{
	return __skb_cow_1(skb, headroom, skb_header_cloned(skb));
}

static inline struct sk_buff *vlan_insert_tag_1(struct sk_buff *skb,
                                              __be16 vlan_proto, u16 vlan_tci)
{
        struct vlan_ethhdr *veth;

        if (skb_cow_head_1(skb, VLAN_HLEN) < 0) {
                kfree_skb(skb);
                return NULL;
        }
...
static inline struct sk_buff *__vlan_put_tag_1(struct sk_buff *skb,
                                             __be16 vlan_proto, u16 vlan_tci)
{
        skb = vlan_insert_tag_1(skb, vlan_proto, vlan_tci);
        if (skb)
...
defined(CONFIG_MT7610_AP_MESH)
...
#else
#ifdef CONFIG_OPTIMIZED_MEMCPY_IN_GOBINET
	skb = __vlan_put_tag_1(skb, skb->vlan_proto, vir_if_idx);
#else
...
#if defined(CONFIG_RA_HW_NAT_WIFI_NEW_ARCH)
	skb->dev = dst_port[DPORT_GMAC1];	/* we use GMAC1 to send the packet to PPE */
#else
	...
	if (debug_level >= 2)
		    	pr_info("skb->dev = %s\n", skb->dev);
	if ((skb->dev == NULL) || ((skb->dev != dst_port[DPORT_GMAC2]) && (skb->dev != dst_port[DPORT_GMAC1]))) {
#if defined(CONFIG_RA_HW_NAT_WIFI_NEW_ARCH)
		skb->dev = dst_port[DPORT_GMAC1];	/* we use GMAC1 to send the packet to PPE */
#else
		skb->dev = dst_port[DP_GMAC1];	/* we use GMAC1 to send the packet to PPE */
...
uint32_t ppe_extif_pingpong_handler(struct sk_buff *skb)
{
	...
	eth = (struct ethhdr *)skb_mac_header(skb);
	//if(skb->protocol == htons(ETH_P_IPV6)) printk("skb->len=%d\n",skb->len);
	if (eth->h_dest[0] & 1) {
		if (skb->dev != NULL) {
...
#if defined (CONFIG_RA_HW_NAT_WIFI_NEW_ARCH)
	for (i = 0; i < MAX_IF_NUM; i++) {
		if (dst_port[i] != NULL && strncmp(dst_port[i]->name, "eth", 3)
#if defined (CONFIG_RA_HW_NAT_NIC_USB)
			&& strncmp(dst_port[i]->name, "wwan", 4)
#endif
		){
...
#endif
			/* Enter binding state */
			printk("Set binding entry skb->protocol in RX=%d\n",skb->protocol);
...
defined(CONFIG_ARCH_MT7622) || defined(CONFIG_ARCH_MT7623)
		if (IS_IPV4_GRP(entry)) {
			...
			foe_entry->ipv6_3t_route.iblk2.fqos = 0; /* PDMA MODE should not goes to QoS */
#endif
#endif
		}
#endif
...
/*clear mib counter*/
		...
		/* Enter binding state */
		//printk("Set binding entry skb->protocol=%d in TX\n",skb->protocol);
		ppe_set_entry_bind(skb, entry);
...
defined(CONFIG_MT7610_AP_MESH)
	 ...
	 * External (WIFI/WWAN) devices shoud be registered by ppe_dev_register_hook()
	 */
	dev = ra_dev_get_by_name("wwan0");
	if (dev)
	{
		ppe_dev_reg_handler(dev);
		for (i = 0; i < MAX_IF_NUM; i++) {
...
#else
#if defined(CONFIG_SUPPORT_OPENWRT)
		dst_port[DP_GMAC1] = ra_dev_get_by_name("eth0");
#ifdef CONFIG_RAETH_GMAC2
		dst_port[DP_GMAC2] = ra_dev_get_by_name("eth1");
#endif
#else
		dst_port[DP_GMAC1] = ra_dev_get_by_name("eth2");
#ifdef CONFIG_RAETH_GMAC2
		dst_port[DP_GMAC2] = ra_dev_get_by_name("eth3");
#endif
#endif
#if defined(CONFIG_RA_HW_NAT_NIC_USB)
		dst_port[DP_PCI] = ra_dev_get_by_name("pci0");	/* PCI interface name */
		dst_port[DP_USB] = ra_dev_get_by_name("wwan0");	/* USB interface name */
#endif				/* CONFIG_RA_HW_NAT_NIC_USB */
#endif				/* CONFIG_RA_HW_NAT_WIFI_NEW_ARCH */
	} else {
#if defined(CONFIG_RA_HW_NAT_WIFI_NEW_ARCH)
		int j = 0;
		if (dst_port[DPORT_GMAC1])
			dev_put(dst_port[DPORT_GMAC1]);
