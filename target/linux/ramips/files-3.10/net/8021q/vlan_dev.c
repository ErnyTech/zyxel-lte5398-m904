...
#define CONFIG_8021P_REMARK 0
#if defined(CONFIG_8021P_REMARK)
#define QOS_8021p_MARK			SKBMARK_VID_ACTION_M 	/* 4~6 bits used for 802.1p */
#define QOS_8021P_0_MARK		0x08	/* default mark is zero */
#define VLAN_HLEN				4
#define VLAN_ETH_ALEN			6

static int vlanPriRemark(struct sk_buff *skb)
{
	uint8_t encap_mode = 0, encap_len = 0;
	unsigned char ucprio = 0;
	unsigned char uc802prio = 0;
	uint16_t vid=0;
	int copy_len = 0;

	//printk("vlanPriRemark skb->mark=0x%x\n", skb->mark);
	if ( skb->mark & QOS_8021p_MARK ) {
		ucprio = (skb->mark & QOS_8021p_MARK) >> SKBMARK_VID_ACTION_S;
	...
	//printk("vlanPriRemark uc802prio=0x%x\n", uc802prio);
	return uc802prio;
}
#endif
...
#if defined(CONFIG_8021P_REMARK)
		vlanPri = vlanPriRemark(skb);
		vlanPri = vlanPri<<13;
		vlan_tci |= vlanPri;
#endif

#if 1 //JeremyTu.20190527.B  The untag reserve vlan 4089 shouldn't be added to wan interface.
		if (4089 != vlan->vlan_id) {
		skb = __vlan_hwaccel_put_tag(skb, vlan->vlan_proto, vlan_tci);
	}
#else //JeremyTu.20190527	
		skb = __vlan_hwaccel_put_tag(skb, vlan->vlan_proto, vlan_tci);
#endif //JeremyTu.20190527.E
